...
comb <- comb %>% mutate(mean_x = rowMeans(select(comb,left_gaze_acs_x, right_gaze_acs_x)), mean_y = rowMeans(select(comb,left_gaze_acs_y, right_gaze_acs_y)), ra.nm=TRUE)
...
ggplot(comb,aes(mean_x,mean_y))+
  geom_path(aes(colour= time))+
  scale_y_continuous(expand = c(1,0)) + scale_x_continuous(expand = c(0,1))
